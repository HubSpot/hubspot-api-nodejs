...
const hubspot = require('../../../src/client')
...
const getEngagementById = async (engagementId) => {
    ...
    const engagementResponse = await hubspotClient.apiRequest({ 
...
const getAllContacts = async (properties, query) => {
    ...
    do {
        ...
        contacts.push(...contactsResponse.results)
    } while (!_.isNil(after))
...
app.get('/contacts/new', async (req, res) => {
    try {
        ...
        const editableProperties = getEditableProperties(propertiesResponse.results)
...
app.get('/contacts/:id', async (req, res) => {
    try {
        ...
        const contactPropertiesNames = _.map(propertiesResponse.results, 'name')
        ...
        const hubspotEngagements = await Promise.map(hubspotEngagementsResponse.results, (engagement) =>
            getEngagementById(engagement.id),
        )

        const editableProperties = getEditableProperties(propertiesResponse.results)
        const properties = getContactEditableProperties(contactResponse.properties, editableProperties)
...
app.post('/contacts/:id/engagement', async (req, res) => {
    try {
        const id = _.get(req, 'params.id')
        let engagement = _.clone(req.body)
        engagement = _.set(engagement, 'metadata.startTime', toDate(_.get(engagement, 'metadata.startTime')))
        engagement = _.set(engagement, 'metadata.endTime', toDate(_.get(engagement, 'metadata.endTime')))
...
app.get('/properties', async (req, res) => {
    try {
        ...
        const mutableProperties = getMutableProperties(propertiesResponse.results)
...
app.post('/properties', async (req, res) => {
    try {
        ...
        const result = await hubspotClient.crm.properties.coreApi.create(CONTACT_OBJECT_TYPE, req)
...
app.post('/properties/:name', async (req, res) => {
    try {
        ...
        const result = await hubspotClient.crm.properties.coreApi.update(CONTACT_OBJECT_TYPE, name, req)
        console.log('Response from API', result)
...
app.get('/properties/new', async (req, res) => {
    try {
        ...
        res.render('list', { items: getPropertyDetails(), action: '/properties', groups: groupsResponse.results })
    } catch (e) {
        handleError(e, res)
    }
})
...
app.get('/properties/:name', async (req, res) => {
    try {
        ...
        const propertyDetails = getPropertyDetails(propertyResponse)
        res.render('list', {
...
app.get('/export', async (req, res) => {
    try {
        ...
        const contactsPropertiesNames = _.map(propertiesResponse.results, 'name')
        ...
        const csvContent = toCsv(contacts, propertiesResponse.results)
